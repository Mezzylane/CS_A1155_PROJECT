...
This report presents a relational database design for a university room booking service.

## 1. Subject Analysis

### 1.1 Candidate Subjects
...
**Design choice for recurrence**: Either model `BookingOccurrence` as a separate child table of `RecurringSeries`, or fold recurrence fields directly into `Booking` with a nullable `recurring_series_id`. The latter is simpler and preferred for this project — see assumptions below.
...
### 1.4 Unclear / Ambiguous Requirements
...
### 1.5 Likely Application Queries
...
The following entity-relationship diagram shows the core entities, their attributes, and the relationships among them. Crow's-foot notation is used: a single line with one cross-bar represents "one", and a three-pronged fork represents "many".
...
- A **Room** may have zero or more **Equipment** items listed through the **RoomEquipment** junction; each Equipment type can appear in many Rooms (M:N, resolved via RoomEquipment).
- An **Organization** makes zero or more **Bookings**; each Booking belongs to exactly one Organization (1:N).
- An **AppUser** creates zero or more **Bookings**; each Booking is created by exactly one User (1:N).
- A **RecurringSeries** optionally groups zero or more **Bookings**; each Booking may optionally belong to one RecurringSeries (1:N, nullable FK). Bookings with a `NULL` `recurring_series_id` are one-off reservations.
...
Stores bookable spaces inside a building. An enum type `room_type` is used to constrain room categories.
...
- `room_number` is unique per building (enforced by `UNIQUE (building_id, room_number)`).
...
Linking table for the many-to-many relationship between `room` and `equipment`. Each row indicates that a particular room contains a particular equipment type.
...
Stores the recurrence rule for a set of bookings that repeat on a schedule. Individual occurrences are materialised as rows in the `booking` table (see below).
...
Stores individual room reservations. This is the core table. Recurrence is handled by an optional link to `recurring_series`. Individual occurrences in a recurring series are materialised as separate `booking` rows.
